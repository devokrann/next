...
export async function POST(request: NextRequest, { params }: { params: { userId: string } }) {
	try {
		...
		if (!userRecord) {
			return NextResponse.json({ error: "Action not allowed" }, { status: 404, statusText: "User Not Found" });
		}
		...
		try {
			...
			const token = await encrypt({ id: tokenId, otp: otpHash, userId: userRecord.id }, 60 * 60);
		...
		console.error("---> route handler error (resend verification code):", error);
