...
export default function SignUp({ userEmail }: { userEmail?: string }) {
	...
	const form = useForm({
		...
		validate: {
			...
			passwordConfirm: (value, values) =>
				compare.string(values.password, value, "Password")
		}
	});
	...
	const parse2 = (rawData: any) => {
		return {
			otp: rawData.otp,
			email: userEmail ? userEmail : form.values.email
		};
	};
	...
	const handleRequest = async () => {
		try {
			...
			const response = await fetch(
				process.env.NEXT_PUBLIC_API_URL + `/api/auth/verify/resend`,
				{
					method: "POST",
		...
		<>
			<Transition mounted={!verify} transition="fade" duration={0}>
				{(styles) => (
					<div style={styles}>
						<LayoutSection
							id={"partial-form-auth-signup-create"}
							padded
							containerized={"xs"}
						>
			...
			<Transition mounted={verify} transition="fade" duration={0}>
				{(styles) => (
					<div style={styles}>
						<LayoutSection
							id={"partial-form-auth-signup-verify"}
							padded
							containerized={"xs"}
						>
							<Stack gap={40} px={{ md: 40 }}>
