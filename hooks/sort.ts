...
export const useSort = <T>(setList: React.Dispatch<React.SetStateAction<T[]>>) => {
	...
	const sortItems = (field: keyof T) => {
		setOrderMap((prevOrderMap) => {
			...
			if (currentOrder === SortOrder.DEFAULT || currentOrder === SortOrder.DESCENDING) {
				nextOrder = SortOrder.ASCENDING;

				setList((list: T[]) => {
					const validItems = list.filter((item) => item[field] !== null && item[field] !== undefined);
					const nullItems = list.filter((item) => item[field] === null || item[field] === undefined);
